...
export class Warehouse extends YUKA.GameEntity {
  constructor(entityManager) {
    ...
    this.workDuration = 0.4 // duration of a single state in seconds
...
class WorkState extends YUKA.State {
  enter(owner) {
    ...
    */
    ...
    // sort by amount
    owner.producers.sort(function (x, y) {
    ...
    owner.drones.forEach((d) => {
      if (!d.isBusy) {
        d.isBusy = true
        console.log(d)
        owner.assignDrone(d, owner.producers[0])
        owner.producers[0].droneAssigned = true

        d.stateMachine.changeTo(WALK)
        owner.producers.shift()
      }
    })
  }
